...
def plot6(m2_in = 0.7, m1_in = 1.4, period_in = 0.787):
    Q = 500
...
# Ratio of T_em to T2 as a function of mass ratio (m2/m1)
def plot9():
...
def fig1A(inclination_in = 44, disk_angle_in = 3, m2_in = 0.5*1.4, m1_in = 1.4, period_in = 0.787):
    phases = np.linspace(0,1,50)
    yy_3d_asra = np.zeros(len(phases))
    yy_cm = delay_model.timeDelay_sp(phases,setting='cm',inclination_in = inclination_in)
    yy_egg = delay_model.timeDelay_sp(phases,setting='egg',inclination_in = inclination_in)
...
def fig1B(inclination_in = 44, disk_angle_in = 3, m2_in = 0.5*1.4, m1_in = 1.4, period_in = 0.787):
    phases = np.linspace(0,1,50)
    yy_3d_asra = np.zeros(len(phases))
    yy_cm = delay_model.radialVelocity(phases,setting='cm',inclination_in = inclination_in)
    yy_egg = delay_model.radialVelocity(phases,setting='egg',inclination_in = inclination_in)
...
def fig1C(m2_in = 0.7, m1_in = 1.4, period_in = 0.787, inclination_in = 44, annotate = True):
    Q = 500
...
def fig1D(phase = 0.65, m2_in = 0.7, m1_in = 1.4, period_in = 0.787, inclination_in = 44, disk_angle_in = 3, mode = 'plot', Q = 22):
...
def fig2B(phase = 0.65, m2_in = 0.7, m1_in = 1.4, period_in = 0.787, inclination_in = 44, disk_angle_in = 3, mode = 'plot', Q = 22):
    plot_labels = False
    plot_arrows = True
...
def fig5():
    ...
    # ~ echo = '1719128205' # just echo
    # ~ echo = '1719251629' # with K1 constraint
    echo = '1725003719'# with K1 constraint and no eclipse constraint

    # ~ rv = '1718328427' # just rv
    ...
    # ~ both = '1718961088' # test
    # ~ both = '1718959681' # test
    # ~ both = '1718992785' # test
    # ~ both_conn = '1718329872'
    ...
    eclipse_conn = np.loadtxt('eclipse_conn.csv')
    ...
    k1_conn = np.loadtxt('k1_conn.csv', skiprows = 1, delimiter = ',')
...
def fig4_old():
    ...
    sco_1 = '1718314125' # Without inclination constraint
    sco_2 = '1718314299' # With inclination constraint
    
    with open(f'mcmc_results//{sco_1}//sampler.pickle', 'rb') as inp:
        sampler = pickle.load(inp)
    samples_without = sampler.get_chain(discard=50, thin=5, flat=True)


    with open(f'mcmc_results//{sco_2}//sampler.pickle', 'rb') as inp:
        sampler = pickle.load(inp)
    samples_with = sampler.get_chain(discard=50, thin=1, flat=True)
    ...
    corner.hist2d(samples_without[:,2],samples_without[:,1]/samples_without[:,0], levels=[0.393,0.865], smooth = True,\
              plot_datapoints = False, color = 'red', alpha = alpha, label = 'both', no_fill_contours = True, plot_density = True)
              
              
    corner.hist2d(samples_with[:,2],samples_with[:,1]/samples_with[:,0], levels=[0.393,0.865], smooth = True,\
              plot_datapoints = False, color = '#580066', alpha = alpha, label = 'both', no_fill_contours = False, plot_density = True)
              
    # ~ plt.scatter(samples_with[:,2],samples_with[:,1]/samples_with[:,0], s = 1, color = 'black')
    ...
    with open(f'mcmc_results//{sco_2}//sampler.pickle', 'rb') as inp:
        sampler = pickle.load(inp)
    samples_with = sampler.get_chain()
    
    
    print(samples_with.shape)
    
    # ~ plt.plot(samples_with[:,0,2],samples_with[:,0,1]/samples_with[:,0,0])
    step_number = np.arange(0,samples_with.shape[0],1)
    
    N = 4
    for i in range(N):
        I = i*samples_with.shape[1]//N
        plot_colorline(samples_with[:,I,2],samples_with[:,I,1]/samples_with[:,I,0],step_number)
    ...
    # both case
    sco = '1715758871'
    with open(f'mcmc_results//{sco}//sampler.pickle', 'rb') as inp:
        sampler = pickle.load(inp)
    samples_both = sampler.get_chain(discard=50, thin=5, flat=True)

    # rv only case
    sco = '1715757786'
    with open(f'mcmc_results//{sco}//sampler.pickle', 'rb') as inp:
        sampler = pickle.load(inp)
    samples_rv = sampler.get_chain(discard=50, thin=5, flat=True)

    fig, axs = plt.subplots(1, 2)
    
    axs[0].hist(samples_rv[:,1]/samples_rv[:,0], bins = 30, color = 'red')
    axs[0].hist(samples_both[:,1]/samples_both[:,0], bins = 30, color = 'green', alpha = 0.8)
    axs[0].set_xlabel('Mass Ratio (m2/m1)')
    
    axs[1].hist(samples_rv[:,0]+samples_rv[:,1], color = 'red')
    axs[1].hist(samples_both[:,0]+samples_both[:,1], color = 'green', alpha = 0.8)
    axs[1].set_xlabel('Total Mass')
    
    plt.show()
    
    
    plt.hist(samples_both[:,2], color = 'green')
    plt.hist(samples_rv[:,2], color = 'red')
    plt.show()
...
def fig5_old():
    ...
    plt.axhspan(0.21,0.34,color='black',alpha=0.5)
    corner.hist2d(samples[:,2],samples[:,1]/samples[:,0], levels=[0.393,0.865], smooth = True,\
                  plot_datapoints = True, color = 'green', alpha = alpha, label = 'both', no_fill_contours = True, plot_density = True)
    ...
    plt.xlabel(r'Inclination ($^{\circ}$)')
    plt.ylabel(r'M $(m_1 + m_2)$')
    plt.show()
        
    var_labels = [r'$m_1$ ($M_\odot$)',r'$m_2$ ($M_\odot$)',r'$i$ ($^{\circ}$)'][:-1]
    fig = corner.corner(samples[:,:-2], labels = var_labels, quantiles =[0.16, 0.5, 0.84], levels=[0.393,0.865], \
                    show_titles=True, title_kwargs={"fontsize": 12}, smooth = True)
    plt.show()
    
    
    plt.hist(samples[:,1]/samples[:,0], bins = 50)
...
def plot_test():
    with open(f'mcmc_results//1718961088//sampler.pickle', 'rb') as inp:
        sampler = pickle.load(inp)
        
    samples1 = sampler.get_chain(discard=150, thin=1, flat=True)
    
    m1F, m2F, iF, alphaF = samples1[:,0], samples1[:,1], samples1[:,2], samples1[:,3]
    
    a = (constants.G*(m1F+m2F)*constants.M_Sun*(0.15804693 * constants.day**2)/(4*np.pi**2))**(1/3)
    K1 = 1e-3*np.sin(iF*np.pi/180)*2*np.pi*a/(0.15804693 * constants.day) * (m2F/(m1F + m2F))
    
    tt = np.linspace(70,130,1000)
    yy = 1/(np.sqrt(2*np.pi)*11.5)*np.exp(-0.5*((tt-101.5)/(11.5))**2)
    plt.plot(tt, yy)
    
    plt.hist(K1, color = 'black', density = True, bins = 30, alpha = 1, histtype='step')
    plt.xlabel(r'$K_1$')
    plt.show()
...
# ~ plot_test()
# ~ fig2A()
# ~ fig1D(phase = 0.25, disk_angle_in = 0)
# ~ fig2B()
# ~ fig4()
# ~ fig4()
# ~ fig3()
